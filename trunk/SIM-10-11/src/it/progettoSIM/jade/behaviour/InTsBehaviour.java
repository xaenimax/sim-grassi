/*
 * Il behaviour che gestisce le richieste di in mandate da altri 
 * tuplespace
 */
...
public class InTsBehaviour extends CyclicBehaviour {

	private TupleSpace ts;

	public InTsBehaviour(TupleSpace ts) {
		this.ts = ts;
	}

	@Override
	public void action() {
		CommandMatch cm = new CommandMatch(AgentJade.CMDIN + AgentJade.CMDTS);
				...
				if (ts.politicheAttive()) {
					...
					if (upBandFree < 10) {
						...
						reply.addUserDefinedParameter("command", AgentJade.CMDREPLY
								+ AgentJade.CMDIN + AgentJade.CMDTS);
						...
						Tuple tuple = (Tuple) ts.inLocal(template);
						if(tuple != null){
							this.ts.increaseInEx();
